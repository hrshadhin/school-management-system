	...
	public function printsheet($regiNo,$exam,$class)
	{
        ...
        if(count($student)>0) {
            ...
            if (count($student) < 1 || count($merit) < 1) {
                return Redirect::to('/result/search')->with('noresult', 'Result Not Found!');
            } else {
...
//sub group need to implement
                ...
                foreach ($subjects as $subject) {
                    ...
                    $maxMarks = Marks::select(DB::raw('max(total) as highest'))->where('session', '=', $student->session)
                ...
                return View::Make('app.stdgradesheet', compact('student', 'extra', 'meritdata', 'subcollection', 'blextra', 'banglaArray', 'enextra', 'englishArray'));
            }
        }
        else
        {
