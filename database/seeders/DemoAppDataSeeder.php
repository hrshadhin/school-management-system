...
class DemoAppDataSeeder extends Seeder
{
    ...
    private function deletePreviousData(){
        ...
        Result::truncate();
        Leave::truncate();
        DB::table('result_publish')->truncate();
        DB::table('result_combines')->truncate();
    ...
    private function deleteUserData(){
        $userIds = UserRole::where('role_id','!=', AppHelper::USER_ADMIN)->pluck('user_id');
    ...
    private function instituteSettingsData()
    {
        ...
        $data['establish'] = '2010';
        $data['website_link'] = 'https://cloudschoolbd.com';
    ...
    private function teacherData() {
        ...
        $instituteHead = $users = Employee::factory()->count(1)->create(['designation' => $insHeadDesignation]);
        $instituteHead->each(function ($teacher) {
            UserRole::create([
        ...
        $teachers = Employee::factory()->count(5)->create();
    ...
    private function employeeData() {
        $employee = Employee::factory()->create(['role_id' => AppHelper::USER_ACCOUNTANT]);
        UserRole::create(['user_id' => $employee->user_id, 'role_id' => $employee->role_id, 'created_by' => $employee->created_by, 'created_at' => $employee->created_at]);

        $employee = Employee::factory()->create(['role_id' => AppHelper::USER_LIBRARIAN]);
        UserRole::create(['user_id' => $employee->user_id, 'role_id' => $employee->role_id, 'created_by' => $employee->created_by, 'created_at' => $employee->created_at]);

        $employee = Employee::factory()->create(['role_id' => AppHelper::USER_RECEPTIONIST]);
        UserRole::create(['user_id' => $employee->user_id, 'role_id' => $employee->role_id, 'created_by' => $employee->created_by, 'created_at' => $employee->created_at]);
    }

    private function sectionData() {
        Section::factory()->count(5)->create();
        Section::factory()->count(2)->create(['class_id'=> 2]);
        Section::factory()->count(2)->create(['class_id'=> 3]);
        Section::factory()->count(2)->create(['class_id'=> 4]);
        Section::factory()->count(2)->create(['class_id'=> 1, 'name' => 'A']);
        Section::factory()->count(2)->create(['class_id'=> 1, 'name' => 'B']);
        Section::factory()->count(2)->create(['class_id'=> 1, 'name' => 'C']);
    }

    private function subjectData()
    {
        $subjects = Subject::factory()->count(10)->create();
        $subjects->each(function ($sub) {
            $teacher = Employee::where('role_id', AppHelper::USER_TEACHER)->inRandomOrder()->first();
            // Create Pivot
            $sub->teachers()->attach($teacher->id);
        });

        Subject::factory()->hasTeachers(1)->create(['class_id' => 1, 'type' => 1, 'name' => 'Bangla 1st', 'code' => '101']);
        Subject::factory()->hasTeachers(1)->create(['class_id' => 1, 'type' => 1, 'name' => 'Bangla 2nd', 'code' => '102']);
        Subject::factory()->hasTeachers(1)->create(['class_id' => 1, 'type' => 1, 'name' => 'English 1st', 'code' => '107']);
        Subject::factory()->hasTeachers(1)->create(['class_id' => 1, 'type' => 1, 'name' => 'English 2nd', 'code' => '108']);
        Subject::factory()->hasTeachers(2)->create(['class_id' => 1, 'type' => 1, 'name' => 'Math', 'code' => '111']);
        Subject::factory()->hasTeachers(1)->create(['class_id' => 1, 'type' => 1, 'name' => 'Computer', 'code' => '112']);

        Subject::factory()->hasTeachers(1)->create(['class_id' => 2, 'type' => 1, 'name' => 'Bangla 1st', 'code' => '101']);
        Subject::factory()->hasTeachers(1)->create(['class_id' => 2, 'type' => 3, 'name' => 'Magic Study', 'code' => '102']);
        Subject::factory()->hasTeachers(1)->create(['class_id' => 2, 'type' => 1, 'name' => 'English 1st', 'code' => '107']);
        Subject::factory()->hasTeachers(1)->create(['class_id' => 2, 'type' => 3, 'name' => 'Drawing', 'code' => '108']);
        Subject::factory()->hasTeachers(1)->create(['class_id' => 2, 'type' => 1, 'name' => 'Math', 'code' => '111']);
        Subject::factory()->hasTeachers(2)->create(['class_id' => 2, 'type' => 2, 'name' => 'Computer', 'code' => '112']);
    }
    ...
    private function studentData() {
        ...
        foreach ($studentIds as $studentId){
            ...
            Registration::insert([$registrationData]);
            $counter++;
        }

        //now assign subject to class 1 students
        $students = Registration::where('class_id', 1)->get();
        $subjects = Subject::where('class_id', 1)->get()->mapWithKeys(function ($sub) {
            return [$sub->id => ['subject_type' => $sub->getRawOriginal('type')]];
        })->toArray();
    ...
    private function studentAttendance() {
        ...
        $students = Registration::where('class_id', 1)->where('academic_year_id', 1)
            ->get(['id', 'shift'])
    ...
    private function employeeAttendance() {
        ...
        $employees = Employee::where('status', AppHelper::ACTIVE)->get()->reduce(function ($employees, $employee) {
            $employees[$employee->id] = [
                'in_time' => $employee->getRawOriginal('duty_start'),
                'out_time' => $employee->getRawOriginal('duty_end')
            ];
            return $employees;
        });
    ...
    private function examRulesData() {
        ...
        $exam = Exam::where('class_id', $class_id)->orderBy('id', 'asc')->first();
...
//           1 2 7
    ...
    private function examMarksData() {
        ...
        $students = Registration::where('class_id', $class_id)->where('academic_year_id', 1)
            ->get(['id', 'section_id'])
            ->reduce(function ($students, $student) {
        ...
        foreach ($subjects as $subject){
            $examRule = ExamRule::where('exam_id',$exam->id)
    ...
    private function generateResult() {
        ...
        $class_id = 1;
        $acYear = 1;
        $examInfo = Exam::where('class_id', $class_id)->orderBy('id', 'asc')->first();
        ...
        $examRules = ExamRule::where('class_id', $class_id)
            ...
            ->reduce(function ($examRules, $rule){
                $examRules[$rule->subject_id] =[
                    ...
                    'subject_type' => $rule->subject->getRawOriginal('type')
                ];
                return $examRules;
            });
